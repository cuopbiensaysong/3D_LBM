...
def get_autoencoder_model(model_dir):
...
class WrapperAutoencoderKL():
    def __init__(self, autoencoder):
        self.autoencoder = autoencoder
        self.latent_means = torch.tensor([-0.0591, -0.1057,  0.7211]).view(1, -1, 1, 1, 1)
        self.latent_stds = torch.tensor([0.4908, 0.5406, 0.5212]).view(1, -1, 1, 1, 1)
    
    def to(self, device):
        self.autoencoder.to(device)
        self.latent_means = self.latent_means.to(device)
        self.latent_stds = self.latent_stds.to(device)
        return self
    
    def to(self, dtype):
        self.autoencoder.to(dtype)
        self.latent_means = self.latent_means.to(dtype)
        self.latent_stds = self.latent_stds.to(dtype)
        return self
    
    def to(self, device, dtype):
        self.autoencoder.to(device, dtype)
        self.latent_means = self.latent_means.to(device, dtype)
        self.latent_stds = self.latent_stds.to(device, dtype)
        return self
    ...
    def decode_stage_2_outputs(self, z):
        z = z * self.latent_stds + self.latent_means
        decoded_z = self.autoencoder.decode_stage_2_outputs(z)
        return decoded_z
...
def get_autoencoder_model_from_ckpt(ckpt_path):
    ...
    return WrapperAutoencoderKL(autoencoder)



def get_discriminator_model():
    return PatchDiscriminator(spatial_dims=3, num_layers_d=3, num_channels=32, in_channels=1, out_channels=1)


def get_unet_model_from_ckpt(pretrained_ckpt_path, device="cpu"):
...
if __name__ == "__main__":
    ckpt_path = "/home/huutien/sources/GenerativeModels/3D_training/results/3D_training_KL_20251229_101810/last_checkpoint.pth"
    get_autoencoder_model_from_ckpt(ckpt_path)